...
const SESSION_KEY = 'edn-grade-alerts-dismissed'
...
export function StudentGradeWelcomeAlert({ notifications }: Props) {
  const router = useRouter()
  const [open, setOpen] = useState(false)
  const [pending, setPending] = useState<StudentNotificationItem[]>([])

  useEffect(() => {
    const unreadGrades = notifications.filter(n => n.type === 'grade' && !n.is_read)
    const dismissed = getDismissedIds()
    const toShow = unreadGrades.filter(n => !dismissed.has(n.id))
    if (toShow.length > 0) {
      setPending(toShow)
      setOpen(true)
    }
    ...
    <Dialog open={open} onOpenChange={v => { if (!v) void dismiss(false) }}>
      <DialogContent className="max-w-lg gap-0 overflow-hidden border-0 p-0 sm:max-w-xl [&>button.absolute]:hidden">
        <div className="relative bg-gradient-to-br from-[#1B3A6B] via-[#234a82] to-[#1B3A6B] px-6 pb-6 pt-8 text-white sm:px-8 sm:pt-10">
          ...
          <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-[#7AB832] shadow-lg">
            <BookOpen className="h-7 w-7 text-white" />
          </div>

          <DialogTitle className="text-left text-2xl font-bold leading-tight text-white sm:text-3xl">
            {pending.length > 1 ? 'Nouvelles notes disponibles' : primary.title}
          </DialogTitle>
          <DialogDescription className="mt-3 text-left text-base leading-relaxed text-white/90">
            {pending.length > 1
              ? `${pending.length} mises à jour de notes viennent d'être publiées par ton établissement.`
              : primary.body}
          </DialogDescription>
            ...
            <ul className="mt-4 max-h-36 space-y-2 overflow-y-auto rounded-xl bg-white/10 p-3 text-sm text-white/95">
              {pending.slice(0, 5).map(n => (
                <li key={n.id} className="flex gap-2">
                  <Sparkles className="mt-0.5 h-4 w-4 shrink-0 text-[#7AB832]" />
                  <span>{n.body}</span>
                </li>
              ))}
              {pending.length > 5 && (
        ...
        <div className="flex flex-col gap-2 bg-white px-6 py-5 sm:flex-row sm:justify-end sm:px-8">
          ...
          <Button
            type="button"
            className="order-1 bg-[#7AB832] hover:bg-[#6aa32b] sm:order-2"
            onClick={() => void handleViewNotes()}
          >
            Voir mes notes
          </Button>
